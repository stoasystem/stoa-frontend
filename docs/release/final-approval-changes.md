...
|----|-----------------|------|-------|-----------------|----------|----------|--------|-------------|
| FAC-001 | Browser review | Homepage | Eyebrow copy said "when it helps"; reviewer requested "when it is needed" and language sync. | Update English wording and sync German, French, and Italian locale copy. | P1 | Fix in Phase 23 because public hero copy affects trust and terminology. | Fixed pending verification | Verify all four locale homepages in Phase 129/130. |
| FAC-002 | Browser review | Header / global logo | Text logo should use the provided logo image from `/Users/zhdeng/stoa-frontend/img`. | Use the provided STOA logo image consistently in header/footer logo surfaces. | P1 | Fix in Phase 23 because incorrect brand presentation affects public demo credibility. | Fixed pending verification | Verify header/footer logo sizing and rendering on core pages. |
| FAC-003 | Browser review | Footer | Footer and main page colors lacked enough contrast. | Give footer a subtle background contrast from the main page while preserving brand direction. | P1 | Fix in Phase 23 because the footer is visible on public/core pages and affects finish quality. | Fixed pending verification | Verify contrast visually and in final accessibility smoke. |
| FAC-004 | Browser review | Header | Header and main page colors lacked enough contrast. | Give sticky header a subtle background contrast from the main page while keeping navigation readable. | P1 | Fix in Phase 23 because navigation is always visible and impacts trust. | Fixed pending verification | Verify desktop/mobile header readability and no layout break. |
| FAC-005 | Browser review | Global imagery | Other image files should be organized under `/Users/zhdeng/stoa-frontend/img`. | Archive remote/public image assets into the local `img/` folder and update code references. | P2 | Fix accepted because the change is low-risk, improves release stability, and avoids remote image dependency during demo. | Fixed pending verification | Verify affected pages render local images and build bundles them. |

## Launch-Candidate Bug Triage

| ID | Priority | Surface | Description | Decision | Status | Next action |
|----|----------|---------|-------------|----------|--------|-------------|
| BUG23-001 | P1 | Homepage copy | Teacher support eyebrow copy did not match final approval wording. | Fix required; aligned through FAC-001. | Fixed pending verification | Verify locale files and homepage render. |
| BUG23-002 | P1 | Branding | Header/footer used a text logo instead of the provided logo asset. | Fix required; aligned through FAC-002. | Fixed pending verification | Verify logo on `/`, `/contact`, and mobile widths. |
| BUG23-003 | P1 | Header/footer visual finish | Header and footer needed clearer contrast against the main page. | Fix required; aligned through FAC-003 and FAC-004. | Fixed pending verification | Verify contrast and layout in Phase 129/130. |
| BUG23-004 | P2 | Image assets | Public pages depended on remote image URLs rather than local archived assets. | Low-risk fix accepted; aligned through FAC-005. | Fixed pending verification | Verify no external image URLs on key pages and build passes. |
...
- P1 count: 3 grouped blockers, all already fixed in the current working tree and pending verification.
