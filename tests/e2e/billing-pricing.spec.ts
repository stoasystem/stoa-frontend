...
test('pricing is public and billing redirects unauthenticated users', async ({ page }) => {
  ...
  await expect(page.getByText('Tutor-supported Plan')).toBeVisible()
...
test('authenticated user can complete and cancel virtual checkout', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /demo checkout/i })).toBeVisible()

  await page.getByRole('button', { name: /complete virtual checkout/i }).click()
  await expect(page).toHaveURL(/\/billing\/checkout\/success\?plan=family/)
  await expect(page.getByRole('heading', { name: /virtual checkout complete/i })).toBeVisible()
