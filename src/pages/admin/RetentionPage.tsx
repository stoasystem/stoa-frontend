    ...
    <DashboardLayout>
      <PageContainer size="wide" className="p-0">
        <PageHeader
          ...
          description="Placeholder retention workflow for inactive students and at-risk families. No reminder automation is implemented."
        />
        {retentionQuery.data && (
              ...
              <CardContent className="space-y-3">
                {retentionQuery.data.inactiveStudents.map((student) => (
                  <div key={student.id} className="rounded-md border p-4">
                    ...
                    <Button className="mt-3" size="sm" variant="outline" onClick={() => toast.info('Reminder action is a frontend placeholder.')}>
                      Send reminder
                    </Button>
                  </div>
                ))}
              ...
              <CardContent className="space-y-3">
                {retentionQuery.data.atRiskFamilies.map((family) => (
                  <div key={family.id} className="rounded-md border p-4">
                    ...
                    <Button className="mt-3" size="sm" variant="outline" onClick={() => toast.info('Follow-up task is a frontend placeholder.')}>
